...
func (s *Service) RegisterFoodFacility(ctx context.Context, actor domain.Actor, command RegisterSourceCommand) (domain.FoodFacility, error) {
	...
	writeCtx, cancelWrite := repository.DetachedWriteContext(ctx)
	defer cancelWrite()
	err := s.store.WithTx(writeCtx, nil, func(tx *sql.Tx) error {
		if err := repository.InsertFoodFacility(writeCtx, tx, source); err != nil {
			return err
		}
		metadata, _ := json.Marshal(map[string]any{"kind": source.Kind, "timezone": source.Timezone})
		return audit.Insert(writeCtx, tx, domain.AuditEvent{
			ID:             uuid.NewString(),
			OrganizationID: actor.OrganizationID,
