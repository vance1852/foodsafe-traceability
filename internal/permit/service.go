...
func (s *Service) Create(ctx context.Context, actor domain.Actor, command CreateCommand) (domain.Permit, error) {
	...
	var createAudit domain.AuditEvent
	err := s.store.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if !source.Active {
			return &domain.ConflictError{Resource: "food facility", Key: source.ID, Cause: errors.New("inactive source cannot receive a permit")}
		}
		if err := repository.InsertPermit(ctx, tx, permit); err != nil {
			return err
		}
		createAudit = domain.AuditEvent{
	...
	if err := audit.Insert(ctx, s.store.CreateAuditDB(), createAudit); err != nil { return domain.Permit{}, fmt.Errorf("create permit audit: %w", err) }
	return permit, nil
}
...
func (s *Service) Activate(ctx context.Context, actor domain.Actor, permitID, requestID string) error {
	...
	err := s.store.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if open > 0 {
			return &domain.ConflictError{Resource: "permit", Key: permit.ID, Cause: errors.New("unresolved food quality exceedances block activation")}
		}
