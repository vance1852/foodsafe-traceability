...
func newFixture(t *testing.T) *fixture {
	...
	now := time.Now().UTC()
	if err := store.CreateOrganization(context.Background(), "org-1", "Food Safety Authority", now); err != nil {
		t.Fatal(err)
	}
...
func (f *fixture) createSourceGraph(t *testing.T) sourceGraph {
	...
	createdSource, err := f.sources.RegisterFoodFacility(ctx, f.supervisor, source.RegisterSourceCommand{Name: "North Foods Plant", Kind: domain.FacilityProcessing, Timezone: "UTC", RequestID: "request-source"})
	if err != nil {
...
func TestStationCannotCrossProductionZoneSourceBoundary(t *testing.T) {
	f := newFixture(t)
	first := f.createSourceGraph(t)
	second, err := f.sources.RegisterFoodFacility(context.Background(), f.supervisor, source.RegisterSourceCommand{Name: "South Cold Warehouse", Kind: domain.FacilityColdStorage, Timezone: "UTC", RequestID: "request-source-2"})
	if err != nil {
		t.Fatal(err)
	}
	...
	var count int
	if err := f.store.DB().QueryRow(`SELECT COUNT(*) FROM inspection_stations WHERE code = 'CROSS'`).Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != 0 {
...
func TestRemediationPlanApprovalAndActionCompletion(t *testing.T) {
	...
	plan, err := f.remediation.CreatePlan(context.Background(), f.supervisor, remediation.CreatePlanCommand{IncidentID: reported.ID, Title: "Allergen containment", Objective: "Isolate affected lots and verify food quality", BudgetCents: 500000, Actions: []remediation.CreateAction{{IdempotencyKey: "isolate-lots", Description: "Isolate impacted lots"}, {IdempotencyKey: "verify-product", Description: "Verify released product quality"}}, RequestID: "remediation-create"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemediationCreationRollsBackDuplicateActionKeys(t *testing.T) {
	...
	_, err = f.remediation.CreatePlan(context.Background(), f.supervisor, remediation.CreatePlanCommand{IncidentID: reported.ID, Title: "Cold-chain stabilization", Objective: "Restore temperature controls before product release", BudgetCents: 100000, Actions: []remediation.CreateAction{{IdempotencyKey: "same", Description: "Repair refrigeration seal"}, {IdempotencyKey: "same", Description: "Validate temperature mapping"}}, RequestID: "remediation-duplicate"})
	if !errors.Is(err, domain.ErrConflict) {
...
func TestAuditFailureRollsBackSourceRegistration(t *testing.T) {
	...
	_, err := f.sources.RegisterFoodFacility(ctx, f.supervisor, source.RegisterSourceCommand{Name: "Rollback Foods Plant", Kind: domain.FacilityProcessing, Timezone: "UTC", RequestID: "source-rollback"})
	if err == nil {
		t.Fatal("registration unexpectedly succeeded without audit table")
	}
	var count int
	if err := f.store.DB().QueryRowContext(ctx, `SELECT COUNT(*) FROM food_facilities WHERE name = 'Rollback Foods Plant'`).Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != 0 {
...
func TestTenantQueriesDoNotExposeOtherOrganization(t *testing.T) {
	...
	if _, err := f.store.DB().Exec(`INSERT INTO food_facilities(id, organization_id, name, kind, timezone, active, version, created_at, updated_at) VALUES ('source-other', 'org-2', 'Other Facility', 'processing_plant', 'UTC', 1, 1, ?, ?)`, now, now); err != nil {
		t.Fatal(err)
	}
...
func TestDatabaseQueriesReturnWrappedNotFoundErrors(t *testing.T) {
	...
	checks := []func() error{
		func() error {
			_, err := f.store.FoodFacility(context.Background(), f.store.DB(), "org-1", "missing")
			return err
		},
		func() error {
