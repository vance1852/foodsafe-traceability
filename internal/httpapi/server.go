...
import (
	...
	"github.com/vance1852/foodsafe-traceability/internal/middleware"
...
func (s *Server) Handler() http.Handler {
	...
	mux.Handle("GET /v1/facilities", middleware.RequireAuth(s.auth, http.HandlerFunc(s.listSources)))
	mux.Handle("POST /v1/facilities", middleware.RequireAuth(s.auth, http.HandlerFunc(s.registerSource)))
...
func (s *Server) registerSource(response http.ResponseWriter, request *http.Request) {
	...
	created, err := s.sources.RegisterFoodFacility(request.Context(), actor, command)
	if err != nil {
		s.writeError(response, request, err)
		return
	}
