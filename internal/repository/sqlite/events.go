...
func (s *Store) AcknowledgeOutboxDelivery(ctx context.Context, event domain.OutboxEvent, now time.Time) error {
	if event.Status != domain.OutboxSending || event.LeaseOwner == "" || event.LeaseToken == "" {
		return &domain.LeaseError{Resource: "outbox event " + event.ID, Owner: event.LeaseOwner, Generation: event.LeaseGeneration}
	}
	if err := s.CompleteOutboxEvent(ctx, event, now); err != nil {
		return fmt.Errorf("acknowledge outbox delivery: %w", err)
	}
	return nil
}
