...
func openTestStore(t *testing.T) *Store {
	t.Helper()
	path := filepath.Join(t.TempDir(), "foodsafe-test.db")
...
func seedOwnershipGraph(t *testing.T, store *Store) {
	...
	source := domain.FoodFacility{ID: "source-1", OrganizationID: "org-1", Name: "North Foods Plant", Kind: domain.FacilityProcessing, Timezone: "UTC", Active: true, Version: 1, CreatedAt: now, UpdatedAt: now}
