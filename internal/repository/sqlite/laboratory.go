...
func (s *Store) CommitLabSubmission(ctx context.Context, organizationID, resultID, analystID string, supervise bool, now time.Time) error {
	return s.WithTx(ctx, nil, func(tx *sql.Tx) error {
		result, err := s.LabResult(ctx, tx, organizationID, resultID)
		if err != nil {
			return err
		}
		if result.AnalystUserID != analystID && !supervise {
			return domain.ErrForbidden
		}
		if err := result.CanTransition(domain.LabResultSubmitted, ""); err != nil {
			return err
		}
		return s.TransitionLabResult(ctx, tx, result, domain.LabResultSubmitted, "", now)
	})
}
