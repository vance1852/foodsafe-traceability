...
func (s *Service) CreatePlan(ctx context.Context, actor domain.Actor, command CreatePlanCommand) (domain.SamplingPlan, error) {
	...
	station, err := s.store.InspectionStation(ctx, s.store.DB(), actor.OrganizationID, command.StationID)
	if err == nil && (station.FacilityID != command.FacilityID || !station.Active) {
		err = &domain.ConflictError{Resource: "inspection station", Key: station.ID, Cause: errors.New("station is inactive or belongs to another source")}
	}
	assignee, assigneeErr := s.store.UserByID(ctx, s.store.DB(), actor.OrganizationID, command.AssignedUserID)
	if err == nil {
		err = assigneeErr
	}
	if err == nil && (!assignee.Active || (assignee.Role != domain.RoleFieldOperator && assignee.Role != domain.RoleSafetySupervisor)) {
		err = &domain.ConflictError{Resource: "sampling assignee", Key: assignee.ID, Cause: errors.New("assignee is not an active field operator")}
	}
	if err == nil {
		var overlaps int
		overlaps, err = s.store.CountOverlappingPlans(ctx, s.store.DB(), station.ID, plan.WindowStart, plan.WindowEnd, "")
		if err == nil && overlaps >= 3 {
			err = domain.ErrCapacityExceeded
		}
	}
	if err == nil {
